    ...
    for(int i=0;i<aantalKnopen-1;i++) {
        for(int j=0;j<aantalTakken;j++) {            
            ...
            < afstand[zoek_index(takken[j]->dest)])) {
                ...
                veranderWaardes(i, zoek_index(takken[j]->dest), afstand[zoek_index(takken[j]->source)] + takken[j]->pLineEdit->text().toInt());
            }//if
        }//for
    }//for
    return;
        ...
        while (rij != NULL) {
            ...
            if (!vastGezet[index_dest]) {
                ...
                if (afstand[index_dest] > afstand[index_source] + takAfstand){
                    ...
                    veranderWaardes(i,index_dest, afstand[index_source] + takAfstand);
                }//if
            }//if
            rij = rij->next;
        }//while
...
void Graaf::veranderWaardes( int i, int j, int x){
    for (; i < aantalKnopen; i++)
        stappenWaardes[i][j] = x;
}//veranderWaardes
